...
export function Chronology({ entries, emptyLabel }: { entries: ChronologyEntry[]; emptyLabel: string }) {
  ...
  return (
    <div className="relative pl-6">
      <div className="absolute left-2 top-1 h-full w-px bg-[rgba(18,20,24,0.12)]" aria-hidden="true" />
      <div className="flex flex-col gap-4">
        {entries.map((entry) => (
          <div key={entry.id} className="relative">
            <div className="absolute -left-6 top-2 h-3 w-3 rounded-full border border-[rgba(127,29,45,0.55)] bg-[rgba(127,29,45,0.8)]" aria-hidden="true" />
